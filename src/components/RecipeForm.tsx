...
const RecipeForm = ( props:RecipeFormProps) => {
  ...
  const onSubmit = (data: any, event: any) => {
    ...
    if (props.id && props.id.length > 0) {
      ...
      // dispatch(choosePrepTime(data.prep_time));
      // dispatch(chooseRiseTime(data.rise_time));
      // dispatch(chooseBakeTime(data.bake_time));
    ...
    <div>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex p-1">
          <Button className="flex justify-start m-3 bg-yellow-900 p-2 rounded hover:bg-slate-800 text-white"
          >
            Submit
          </Button>
        </div>
